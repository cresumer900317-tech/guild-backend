...
app.add_middleware(
    CORSMiddleware,
    # 친구들.com(=xn--2e0br5l24w.com, icp.=ICP 코드브릿지), jisoar.com(개인 브랜드·/hq 개인업무앱 이전), *.github.io, localhost 만 허용.
    allow_origin_regex=r"^https://(www\.|icp\.)?xn--2e0br5l24w\.com$|^https://(www\.)?jisoar\.com$|^https://cresumer900317-tech\.github\.io$|^http://(localhost|127\.0\.0\.1)(:\d+)?$",
    allow_methods=["*"],
    allow_headers=["*"],
)
